  ...
  return (
    <div>
      <div style={{ marginBottom: 20, paddingBottom: 16, borderBottom: "1px solid var(--bdr)" }}>
        <span style={{ fontFamily: "'Inter_28pt-Regular',sans-serif", fontSize: 9, letterSpacing: ".18em", color: "var(--gold)", textTransform: "uppercase", marginBottom: 6, display: "block", opacity: .7 }}>Step 05 / 05</span>
        <h1 className="serif" style={{ fontSize: 30, fontWeight: 300, lineHeight: 1.15, color: "var(--paper)" }}>
          Review &amp; <span style={{ color: "var(--gold)", fontWeight: 700, fontFamily: "'Inter_24pt-Medium',sans-serif" }}>generate</span>
        </h1>
      ...
      <BlurFade delay={0}>
        <Card>
          <div style={{ padding: "10px 14px", borderBottom: "1px solid var(--bdr2)", display: "flex", alignItems: "center", gap: 8, background: "linear-gradient(180deg,rgba(255,255,255,.025) 0%,transparent 100%)" }}>
            ...
            <span style={{ marginLeft: "auto", fontSize: 9.5, fontFamily: "'Inter_28pt-Regular',sans-serif", color: "var(--steel2)" }}>⏱ ~45 seconds</span>
          </div>
            ...
            <div style={{ display: "flex", gap: 10 }}>
              {[
                ...
                <div key={i} style={{ flex: 1, display: "flex", flexDirection: "column", gap: 6 }}>
                  ...
                  <span style={{ fontSize: 8.5, color: "var(--steel2)", fontFamily: "'Inter_28pt-Regular',sans-serif", textAlign: "center", letterSpacing: ".04em" }}>{item.caption}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        <Card>
          ...
          <div style={{ padding: 14 }}>
              ...
              {INTENTS.map(intent => (
                <div key={intent.label}
                  ...
                  <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 7 }}>
                    ...
                    <span style={{ fontFamily: "'Inter_28pt-Regular',sans-serif", fontSize: 11, fontWeight: 600, color: state.intent === intent.label ? "var(--gold)" : "var(--steel)", background: state.intent === intent.label ? "var(--gold-dim2)" : "var(--panel2)", border: `1px solid ${state.intent === intent.label ? "var(--gold-bdr)" : "var(--bdr)"}`, borderRadius: 5, padding: "2px 7px" }}>{intent.qty} visuals</span>
                  </div>
            ...
            <div style={{ border: "1.5px dashed var(--bdr)", borderRadius: "var(--r-lg)", padding: "11px 14px", display: "flex", alignItems: "center", justifyContent: "space-between", background: "var(--panel)", marginBottom: 14 }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 5 }}>
                <input type="number" min={1} max={150} value={state.qty}
                  ...
                  style={{ width: 48, background: "var(--panel2)", border: "1px solid var(--bdr)", borderRadius: 6, color: "var(--paper)", fontFamily: "'Inter_28pt-Regular',sans-serif", fontSize: 15, fontWeight: 600, outline: "none", textAlign: "center", padding: "4px 5px" }} />
            ...
            <div style={{ marginTop: 0 }}>
              <span style={{ fontSize: 8.5, fontWeight: 600, letterSpacing: ".14em", textTransform: "uppercase", color: "var(--steel2)", fontFamily: "'Inter_28pt-Regular',sans-serif", marginBottom: 9, display: "block" }}>
              ...
              <div style={{ display: "flex", gap: 4, flexWrap: "wrap" }}>
                {QTY_BTNS.map(n => (
                  <div key={n} onClick={() => update({ qty: n, intent: INTENTS.find(i => i.qty === n)?.label ?? state.intent })}
                    style={{ flex: 1, minWidth: 36, padding: "8px 4px", borderRadius: 7, border: `1px solid ${state.qty === n ? "var(--gold-bdr2)" : "var(--bdr)"}`, background: state.qty === n ? "var(--gold-dim2)" : "var(--panel)", color: state.qty === n ? "var(--gold)" : "var(--steel2)", fontSize: 11, fontFamily: "'Inter_28pt-Regular',sans-serif", cursor: "pointer", textAlign: "center", transition: "all .17s" }}>
                    {n}
                  </div>
                ))}
              </div>
      ...
      <BlurFade delay={0.1}>
        <Card>
          ...
          <div style={{ padding: 14 }}>
            <span style={{ fontSize: 8.5, fontWeight: 600, letterSpacing: ".14em", textTransform: "uppercase", color: "var(--steel2)", fontFamily: "'Inter_28pt-Regular',sans-serif", marginBottom: 10, display: "block" }}>Aspect Ratio</span>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 6 }}>
              {FORMATS.map(f => (
                <div key={f.ratio} onClick={() => update({ format: f.ratio, formatLabel: f.name })}
                  ...
                  <span style={{ fontFamily: "'Inter_28pt-Regular',sans-serif", fontSize: 14, fontWeight: 600, color: state.format === f.ratio ? "var(--gold)" : "var(--paper)", display: "block", lineHeight: 1, marginBottom: 4 }}>{f.ratio}</span>
      ...
      <BlurFade delay={0.15}>
        <Card>
          ...
          <div style={{ padding: 14 }}>
            <span style={{ fontSize: 8.5, fontWeight: 600, letterSpacing: ".14em", textTransform: "uppercase", color: "var(--steel2)", fontFamily: "'Inter_28pt-Regular',sans-serif", marginBottom: 10, display: "block" }}>Resolution</span>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 6, marginBottom: 12 }}>
              {QUALITIES.map(q => (
                <div key={q.res} onClick={() => update({ quality: q.res, qualityLabel: q.label, qualityMult: QUALITY_MULTS[q.res] })}
                  ...
                  {q.badge && <div style={{ position: "absolute", top: 5, right: 5, fontSize: 7, fontFamily: "'Inter_28pt-Regular',sans-serif", padding: "1px 5px", borderRadius: 8, fontWeight: 600, letterSpacing: ".06em", background: "rgba(201,168,76,.15)", color: q.badgeColor, border: `1px solid ${q.badgeColor}40` }}>{q.badge}</div>}
                  <span style={{ fontFamily: "'Inter_28pt-Regular',sans-serif", fontSize: 18, fontWeight: 700, color: state.quality === q.res ? "var(--gold)" : "var(--paper)", display: "block", lineHeight: 1, marginBottom: 3 }}>{q.res}</span>
                  <span style={{ fontSize: 8, color: "var(--steel2)", display: "block", marginBottom: 2, fontFamily: "'Inter_28pt-Regular',sans-serif", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{q.px}</span>
                  <span style={{ fontSize: 9, color: "var(--steel2)", display: "block" }}>{q.label}</span>
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 4, marginTop: 5, fontSize: 9, fontFamily: "'Inter_28pt-Regular',sans-serif", color: state.quality === q.res ? "var(--gold)" : "var(--steel2)", opacity: state.quality === q.res ? .8 : 1 }}>
                    {q.mult} credit
                  </div>
                </div>
              ))}
        ...
        <Card>
          ...
          <div style={{ padding: 14 }}>
            ...
            <div style={{ background: "var(--ink2)", border: "1px solid var(--bdr)", borderRadius: 9, padding: "12px 14px", marginTop: 12 }}>
              <div style={{ fontSize: 8, letterSpacing: ".14em", textTransform: "uppercase", color: "var(--steel2)", fontFamily: "'Inter_28pt-Regular',sans-serif", marginBottom: 9 }}>Credit Calculation</div>
              ...
              <div style={{ display: "flex", justifyContent: "space-between", fontSize: 13, color: "var(--paper)", fontWeight: 600, marginTop: 8, paddingTop: 8, borderTop: "1px solid var(--bdr)", fontFamily: "'Inter_28pt-Regular',sans-serif" }}>
                <span>Total credits</span><span style={{ color: "var(--gold)" }}>{totalCost}</span>
              </div>
              <div style={{ fontSize: 9.5, color: "var(--steel2)", marginTop: 5, textAlign: "right", fontFamily: "'Inter_28pt-Regular',sans-serif" }}>{remaining} remaining after this shoot</div>
      ...
      <BlurFade delay={0.25}>
        <div style={{ background: "rgba(201,168,76,.03)", border: "none", borderLeft: "1px solid rgba(201,168,76,.35)", borderRadius: 3, padding: "4px 10px", fontSize: 10, color: "var(--steel2)", lineHeight: 1.55 }}>
          <strong style={{ color: "rgba(201,168,76,.7)", fontWeight: 500 }}>Preview guarantee:</strong> 2 preview images first at no extra cost. Approve before the full run — credits are never burned on bad output.
        </div>
      </BlurFade>
    </div>
  )
